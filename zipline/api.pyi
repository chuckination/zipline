...
def read_csv(filepath, pre_func=None, post_func=None, date_column='date', date_format=None, timezone='UTC', symbol=None, mask=True, symbol_column=None, special_params_checker=None, **kwargs):
    """Fetch a csv from a local filepath and register the data so that it is
    queryable from the ``data`` object.

    Parameters
    ----------
    filepath : str
        The filepath of the local csv file to load.
    pre_func : callable[pd.DataFrame -> pd.DataFrame], optional
        A callback to allow preprocessing the raw data returned from
        read_csv before dates are paresed or symbols are mapped.
    post_func : callable[pd.DataFrame -> pd.DataFrame], optional
        A callback to allow postprocessing of the data after dates and
        symbols have been mapped.
    date_column : str, optional
    ...
    date_format : str, optional
        The format of the dates in the ``date_column``. If not provided
        ``read_csv`` will attempt to infer the format. For information
        about the format of this string, see :func:`pandas.read_csv`.
    timezone : tzinfo or str, optional
        The timezone for the datetime in the ``date_column``.
    symbol : str, optional
        If the data is about a new asset or index then this string will
        be the name used to identify the values in ``data``. For example,
        one may use ``read_csv`` to load data for VIX, then this field
        could be the string ``'VIX'``.
    mask : bool, optional
        Drop any rows which cannot be symbol mapped.
    symbol_column : str
    ...
    **kwargs
    ...
    Returns
    -------
    csv_data_source : zipline.sources.requests_csv.PandasFileCSV
        A local csv file source that will pull data from the filepath specified.
    """
